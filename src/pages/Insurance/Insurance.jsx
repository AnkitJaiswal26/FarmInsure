...
const Insurance = () => {
	...
	const fetchIns = useCallback(async () => {
		try {
			const contractAddress = window.location.pathname.split("/")[1];
	...
	const fetchLocation = useCallback(async () => {
		try {
			...
			for (let i = 0; i < farms.length; i++) {
				...
				areas[farms[i].location.toString()] =
					farms[i].landArea.toString();
			}
			setLocation(arr);
			setAreas(areas);
